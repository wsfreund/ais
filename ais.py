...
class AIS(object):
  ...
  def forward_ais(self, x, z = None, schedule = None):
    """
    ...
    self._l_const_x            = tf.constant(x, dtype=tf.float32)
    ...
    # space
    self._l_z       = tf.Variable( self._generator_wrapper.sample_latent(
    ...
    final_state = tf.reshape( self._l_z, desired_shape + (self._generator_wrapper.latent_dim(),) )
  ...
  def backward_ais(self, x, z):
    ...
    self._nchains = 1
    # Add noise to x
    x = tf.add(x, tf.random.normal( x.shape, stddev=tf.sqrt(self._observation_model.var) ) )
    ...
    posterior = tf.multiply( t,  
      ...
      tf.reshape( 
        self._observation_model(
          ...
          tf.reshape( self._generator_wrapper.transform(z)
